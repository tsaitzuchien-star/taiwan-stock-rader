...
# 1. 頁面與環境設定
# ==========================================
st.set_page_config(page_title="台股妖股雷達 V10.5.1 | 完美偽裝版", layout="wide", page_icon="🏢")
...
with st.sidebar:
    st.markdown("### 🏢 總裁控制台")
    st.markdown("<p style='color:#8b92a5; font-size:0.9em;'>若情報網卡住或出現連線異常，請點擊下方按鈕強制重啟情報探測器，清除不良記憶。</p>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("### 👨‍💻 開發日誌")
    st.markdown("- **V10.5.1:** API 全套偽裝裝甲\n- **V10.5:** API 直連核彈版(含 VIP Token)\n- **V10.4.2:** 同學會極限測謊(零容忍)\n- **V10.4.1:** SSL 隱形通行證\n- **V10.3:** 預設參數客製化綁定")
...
# 6. 國庫風控面板 
# ==========================================
st.markdown("<h1>🏢 台股妖股雷達 <span style='color: #FFD700;'>V10.5.1</span> <span style='font-size: 0.5em; color: #8b92a5;'>(完美偽裝版)</span></h1>", unsafe_allow_html=True)
...
rc3.metric("🚦 系統狀態", "V10.5.1 API 偽裝上線", delta="無聲潛入敵方金庫", delta_color="normal")
st.markdown("</div>", unsafe_allow_html=True)

# ==========================================
# 7. 情報工具箱 (包含 V10.5.1 API 完美偽裝防護網)
# ==========================================
...
def check_cmoney_blind_spot(stock_id):
    """🚀 V10.5.1 完美偽裝版：補齊全套 Headers，徹底騙過 Kong 防火牆"""
    ...
    # 🛡️ 極限偽裝：把所有真實瀏覽器會發送的身分證都帶上
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
        'Accept': 'application/json, text/plain, */*',
        'Accept-Language': 'zh-TW,zh;q=0.9,en-US;q=0.8,en;q=0.7',
        'Origin': 'https://www.cmoney.tw',
        'Referer': f'https://www.cmoney.tw/forum/stock/{stock_id}', # 騙伺服器我們是從該檔股票頁面進來的
    ...
    try:
        ...
        if res.status_code == 401 or res.status_code == 403:
            return False, f"🟡 授權遭拒 (HTTP {res.status_code})：Token 可能已過期或被鎖定。"
            
        if res.status_code != 200:
            return False, f"🟡 API 連線受阻 (HTTP {res.status_code})：防火牆擋下了這次探測。"
        ...
        return False, "✅ 隱藏 API 測謊通過！(已深層掃描最新留言，無套牢跡象)"
...
# 🏢 V10.5.1 核心功能：AI 全自動面試考核 (整合 API 測謊)
...
with tab2:
    ...
    st.markdown("<p style='color:#8b92a5;'>總司令，請在此直接輸入您關注的股票代號。系統將繞過雷達，瞬間回傳情報與 K 線圖，並啟動 AI 自動面試評估系統。</p>", unsafe_allow_html=True)
